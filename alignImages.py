...
def displaysEqualizationHistograms(
    I_histogram, lower_threshold, outputFileName, log1, verbose=False
):
    ...
    writeString2File(
        log1.fileNameMD,
        "{}\n ![]({})\n".format(
            os.path.basename(outputFileName), outputFileName + "_intensityHist.png"
        ),
        "a",
    )
...
def showCCimage(
    image1_uncorrected, image2_uncorrected, outputFileName, shift, verbose=False
):
    image_product = (
...
def saveImageAdjusted(fileName, fileNameMD, image1):
    ...
    writeString2File(
        fileNameMD,
        "{}\n ![]({})\n".format(os.path.basename(fileName), fileName + "_adjusted.png"),
        "a",
    )
    plt.close()
...
def align2Files(fileName, imReference, param, log1, session1, dataFolder, verbose):
    ...
    outputFileName = (
        ...
        + os.sep
        + os.path.basename(fileName2).split(".")[0]
    )
    ...
    log1.report(
        "Lower threshold for {}: {:.2f}".format(
            os.path.basename(outputFileName + "_ref"), lower_threshold["Im1"]
        )
    )
    log1.report(
        "Lower threshold for {}: {:.2f}".format(
            os.path.basename(outputFileName), lower_threshold["Im2"]
        )
    )
    ...
    displaysEqualizationHistograms(
        I_histogram, lower_threshold, outputFileName, log1, verbose
    )
    ...
    writeString2File(
        log1.fileNameMD,
        "{}\n ![]({})\n".format(
            os.path.basename(outputFileName), outputFileName + "_overlay_corrected.png"
        ),
        "a",
    )
...
def alignImages(param, log1, session1):
    ...
    if param.param["alignImages"]["operation"] == "overwrite":
        ...
        for currentFolder in dataFolder.listFolders:
            ...
            positionROIinformation = param.param["acquisition"][
                "positionROIinformation"
            ]
            referenceBarcode = param.param["alignImages"]["referenceFiducial"]
            fileNameReferenceList, ROIList = RT2fileName(
                param.fileList2Process, referenceBarcode, positionROIinformation
            )
            if len(fileNameReferenceList) > 0:
                ...
                for fileNameReference in fileNameReferenceList:
                    ...
                    imReference = Image()
                    imReference.loadImage2D(
                        fileNameReference, log1, dataFolder.outputFolders["zProject"]
                    )
                    ...
                    for fileName in param.fileList2Process:
                        ...
                        if (fileName not in fileNameReference) and os.path.basename(
                            fileName
                        ).split("_")[positionROIinformation] == ROI:
                ...
                log1.report(
                    "Reference Barcode file does not exist: {}",
                    format(referenceBarcode),
                )

        alignmentResultsTable.write(
            dataFolder.outputFiles["alignImages"].split(".")[0] + ".dat",
            format="ascii.ecsv",
            overwrite=True,
        )
            ...
            if len(param.fileList2Process) > 0:
                ...
                for fileName in param.fileList2Process:
                    ...
                    if shiftArray != None:
                        ...
                        Im = Image()
                        Im.loadImage2D(
                            fileName, log1, dataFolder.outputFolders["zProject"]
                        )
                        ...
                        shiftArray == None
                        and label == param.param["alignImages"]["referenceFiducial"]
                    ):
                        Im = Image()
                        Im.loadImage2D(
                            fileName, log1, dataFolder.outputFolders["zProject"]
                        ...
                        log1.report(
                            "Saving image for referenceRT ROI:{}, label:{}".format(
                                ROI, label
                            ),
                            "Warning",
                        )
                        ...
                        log1.report(
                            "No shift found in dictionary for ROI:{}, label:{}".format(
                                ROI, label
                            ),
                            "Warning",
                        )
