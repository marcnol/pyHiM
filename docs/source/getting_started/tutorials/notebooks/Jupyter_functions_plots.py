...
def copy_localization_table(Input_folder):
    ...
    dest = os.path.join(
        Input_folder, "localize_3d", "data", "localizations_3D_barcode.dat"
    )
    os.makedirs(os.path.join(Input_folder, "localize_3d", "data"))
...
def plot_segment_object(Input_folder, titles, datatype, RTs_references=""):
    ...
    if datatype == "DAPI" or datatype == "RT":
        ...
        imgs = [x[65:1840, 170:1940] for x in imgs]
        imgs.append(imgs[0][840:1020, 520:700])
...
def plot_matrix(input_folder, data_type="proximity"):
    ...
    # Create list of images
    if data_type == "PWD":
        files = glob.glob(input_folder + "tracing" + os.sep + "*_PWDmatrixKDE.png")
    elif data_type == "proximity":
        files = glob.glob(input_folder + "tracing" + os.sep + "*HiMmatrix.png")
