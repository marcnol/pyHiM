...
class FilterTraces:
    ...
    def trace_statistics(self, save=True, tag=""):
        ...
        ax1.bar(bc_unique, barcode_detection_duplicated_stat, width=0.8, bottom=barcode_detection_single_stat,
        ...
        ax2.bar(efficacy_unique, efficacy_w_duplicates_stat, width=3, bottom=efficacy_wo_duplicates_stat,
    ...
    def clustering(trace_data, radius_min, radius_max, verbose=False):
        ...
        neighbors = sorted(neighbors, key=lambda a: len(a),
                           reverse=True)  # sort neighbors based on number of detections
        ...
        for loc in neighbors:
            ...
            for n, cluster in enumerate(clusters):
                intersect = set(cluster).intersection(
                    loc.tolist())  # check common localizations between loc and cluster
                if len(intersect) > 0:
                    new_cluster = False
    ...
    def detect_overlapping_barcodes(self, trace_data, verbose=False, save=True):
        ...
        for n, id_to_keep in enumerate(spot_id_to_keep):
            ...
            x = (trace_data.loc[trace_data["Spot_ID"] == id_to_keep, 'x'].iloc[0] +
                 trace_data.loc[trace_data["Spot_ID"] == id_to_remove, 'x'].iloc[0]) / 2
            y = (trace_data.loc[trace_data["Spot_ID"] == id_to_keep, 'y'].iloc[0] +
                 trace_data.loc[trace_data["Spot_ID"] == id_to_remove, 'y'].iloc[0]) / 2
            z = (trace_data.loc[trace_data["Spot_ID"] == id_to_keep, 'z'].iloc[0] +
                 trace_data.loc[trace_data["Spot_ID"] == id_to_remove, 'z'].iloc[0]) / 2
                ...
                if repeated_bc:
                    ...
                    for bc in repeated_bc:
                        if len(trace_data[trace_data["Barcode #"] == bc]) > 1:
                            new_trace_data.drop(new_trace_data[(new_trace_data['Barcode #'] == bc) &
                                                               (new_trace_data['Trace_ID'] == trace_id)].index,
                                                inplace=True)
                else:
