        ...
        for currentFolder in dataFolder.listFolders:
           ...
           # generates lists of files to process    
            ...
            if len(param.fileList2Process)>0:                        
                fileList2Process=[file for file in glob.glob(dataFolder.outputFolders['segmentedObjects']+os.sep+'*')
                ...
                if processingList['cleanAllMasks']:
                    ...
                    for fileName in fileList2Process:
                        os.remove(fileName)
                        log1.report('Removing SND mask: {}'.format(os.path.basename(fileName)),'info')
                    ...
                    for fileName in param.fileList2Process:
                        ...
                        registeredFileName = dataFolder.outputFolders['alignImages']+os.sep+\
                                            os.path.basename(fileName).split('.')[0]+'_2d_registered.npy'
    
                        if os.path.exists(registeredFileName):
                            outputFileName=dataFolder.outputFolders['segmentedObjects']+os.sep+\
                                os.path.basename(registeredFileName).split('.')[0]+'_SNDmask'+'_'+processingList['addMask']+'.npy'
                            createsUserMask(registeredFileName,outputFileName)
                            numberMaskedFiles+=1
                            log1.report('Segmented image for SND channel ROI#{}: {}'.format(ROI,outputFileName),'info')
                        else:
                            log1.report('Could not find registered image for SND channel:{}'.format(registeredFileName),'error')
...
def assignsSNDmask2Cells(fileList2Process,positionROIinformation):
    ...
    for fileName in fileList2Process:
        ...
        fileNameDAPImask=os.path.dirname(fileName)+os.sep+ "_".join(os.path.basename(fileName).split('_')[0:7])+'_ch00_Masks.npy'
        if os.path.exists(fileNameDAPImask):
            ...
            log1.report('Matched DAPI and SND masks: \n SND: {}\n DAPI: {}\n'.format(fileName,fileNameDAPImask),'info')
            numberFilesProcessed+=1
            
        else:
            
            log1.report('Could not find expected DAPI mask: {}'.format(fileNameDAPImask),'warning')
...
if __name__ == '__main__':
    ...
    writeString2File(log1.fileNameMD,"# Process SND channel {}".format(now.strftime("%d/%m/%Y %H:%M:%S")),'w') # initialises MD file
