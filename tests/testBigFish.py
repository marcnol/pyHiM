...
plot.plot_detection(image2D, spots_post_decomposition, radius=radius_yx, 
...
spots_subpixel = detection.fit_subpixel(image3D, spots, voxel_size_z=voxel_size_z, voxel_size_yx=voxel_size_yx,
...
sources, im1_bkg_substracted = _segmentSourceInhomogBackground(image2D,
                                                               threshold_over_std, 
                                                               fwhm, 
                                                               brightest, 
                                                               sigma_clip)
...
spotsAstropy_subpixel = detection.fit_subpixel(image3D, spotsAstropy, voxel_size_z=voxel_size_z, voxel_size_yx=voxel_size_yx,
                 psf_z=psf_z, psf_yx=psf_yx)
...
spotsAstropy2_subpixel = detection.fit_subpixel(image3D, spotsAstropy2, voxel_size_z=voxel_size_z, voxel_size_yx=voxel_size_yx,
                 psf_z=psf_z, psf_yx=psf_yx)
...
selection_ASTROPY_3D = np.nonzero((barcodeMapSinglebarcode["xcentroid"]>xPlane-window) & (barcodeMapSinglebarcode["xcentroid"]<xPlane+window))
...
selection_3D_bigfish = np.nonzero((spotsAstropy2_subpixel[:,2]>xPlane-window) & (spotsAstropy2_subpixel[:,2]<xPlane+window))
...
(Y_YZ_fitting, 
 Z_YZ_fitting,
 flux_YZ_fitting,
) = sourcesYZ_fitting['xcentroid'][keepHighFlux], sourcesYZ_fitting['ycentroid'][keepHighFlux],sourcesYZ_fitting['flux'][keepHighFlux]
...
fig=_showsImageSources(image3D_ZY , image3D_ZY , Y_YZ_fitting, Z_YZ_fitting, flux_YZ_fitting, percent=99.5,vmin=0,vmax=flux_YZ_fitting.max())
