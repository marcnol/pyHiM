...
from alignBarcodesMasks import calculateContactProbabilityMatrix
    ...
    for idataSet in dataSets:
        ...
        if runParameters["type"]=='contact' and "TAD2plot" in ListData[idataSet].keys():
            ...
            for isample, ifigure, iFigLabel, yticks, xticks,legend, icolorbar in zip(Samples, FigList, FigLabels, Yticks, Xticks,legendList,colorbar):
                ...
                subMatrix=HiMdata.data["ensembleContactProbability"][TAD2plot[0]:TAD2plot[1],TAD2plot[0]:TAD2plot[1]]
...
def plotHiMLineProfile(ListData,runParameters):
    ...
    for idataSet in dataSets:
        if runParameters["type"]=='contact' and "plotSegment_anchor" in ListData[idataSet].keys():
            ...
            HiMdata.loadData()
            # m1=HiMdata.data["ensembleContactProbability"]
            m1,_=calculateContactProbabilityMatrix(HiMdata.data["SCmatrixCollated"],
                                                       list(HiMdata.data["uniqueBarcodes"]),
            ...
            for iSample, sample in enumerate(Samples):
                ...
                matrix,_=calculateContactProbabilityMatrix(HiMdata.data["SCmatrixCollated"],
                                                           list(HiMdata.data["uniqueBarcodes"]),
                                                           pixelSize=runParameters["pixelSize"],
                                                           threshold=0.25,
                                                           norm="nonNANs")
